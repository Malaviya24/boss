...
const TYPE_CONFIG = {
  jodi: {
    folder: 'jodi',
    pattern: /^(?:\d+-jodi-dpbossss\.boston-jodi-chart-record-)?([a-z0-9-]+)\.php$/i,
  },
  panel: {
    folder: 'panel',
    pattern: /^(?:\d+-panel-dpbossss\.boston-panel-chart-record-)?([a-z0-9-]+)\.php$/i,
  },
};
...
function rewriteMarketPhpLinks(html, { defaultType, knownSlugsByType }) {
  ...
  $('[href]').each((_, element) => {
    ...
    let parsedPathname = '';
    try {
      parsedPathname = new URL(raw, 'https://dpbossss.boston/').pathname;
    } catch {
