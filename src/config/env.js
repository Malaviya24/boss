...
export function loadEnv() {
  ...
  return {
    ...
    scrapeConcurrency: Math.max(1, toInt(process.env.SCRAPE_CONCURRENCY, 4)),
    maxHistoryLength: toInt(process.env.MAX_HISTORY_LENGTH, 50),
    staleAfterMs: toInt(process.env.STALE_AFTER_MS, 1800000),
    redisUrl: process.env.REDIS_URL ?? '',
    queueName: process.env.SCRAPE_QUEUE_NAME ?? 'dpboss-scrape-jobs',
    queuePrefix: process.env.SCRAPE_QUEUE_PREFIX ?? 'dpboss',
    queueJobTimeoutMs: toInt(process.env.SCRAPE_JOB_TIMEOUT_MS, 45000),
    queueJobAttempts: Math.max(1, toInt(process.env.SCRAPE_JOB_ATTEMPTS, 3)),
    queueBackoffMs: toInt(process.env.SCRAPE_BACKOFF_MS, 1500),
