...
export function createMatkaService({ env }) {
  ...
  async function upsertOpenPanel({ marketId, panel, adminUser }) {
    ...
    const normalizedPanel = ensurePanel(panel);
    const todayKey = getCurrentDateKey(timeZone);
    const now = new Date();
    const openAt = getScheduledDateForToday(market.openTime, timeZone);
    const revealAt = now.getTime() < openAt.getTime() ? openAt : now;

    // Check if today's open result was auto-generated — if so, save to tomorrow instead
    const todayResult = await MatkaMarketResultModel.findOne({
      marketId: market._id,
      resultDate: todayKey,
    });

    const isAutoGenOpen = todayResult?.autoGenerated?.open === true;
    const dateKey = isAutoGenOpen ? getTomorrowDateKey(timeZone) : todayKey;
  ...
  async function upsertClosePanel({ marketId, panel, adminUser }) {
    ...
    // Check if today's close result was auto-generated — if so, save to tomorrow instead
    const todayResult = await MatkaMarketResultModel.findOne({
      marketId: market._id,
      resultDate: todayKey,
    });

    const isAutoGenClose = todayResult?.autoGenerated?.close === true;
    const dateKey = isAutoGenClose ? getTomorrowDateKey(timeZone) : todayKey;
