...
function parseJodiValue(resultText) {
  return parseResultParts(resultText).jodi;
}
...
export async function scrapeJodiValue({ browser, url, timeoutMs, logger }) {
  if (!url) {
    return '';
  }
  ...
  try {
    ...
    logger?.warn('jodi_scrape_puppeteer_failed', { url, message: error.message });
